...
public class ReservationViewAction extends ActionSupport implements SessionAware {
	...
	public String execute() {
		...
		if (session.containsKey("loginFlg") == true) {
			...
			for (int i = 0; i < 7; i++) {
				...
				case 0:
					...
					reservationSearch = dao.display(Day.get(0));
					break;
				case 1:
					Day = addDay(nowDay, 1);
		...
		/**
		 * if(reservationSearch.size() >= 0){ result = SUCCESS; }
		 */
		return result;
	}

	/**
	 * addDayメソッド nextWeekFarstDayに1週間後の日付けを格納し、DayリストにString型で格納、
	 * さらにnextWeekFarstDayの1日後の日付けを 代入する。それを7回繰り返すメソッド。
	 */
